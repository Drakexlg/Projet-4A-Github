...
Mat remove_noise(Mat img, int a, int b, int c, int d)
{
	...
	//erode : supprime les petits groupes de pixels noirs isolés
	// l'effet de la zone peut etre changer avec la valeur du cv::Size
	erode(difference, difference, cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(a, a)));  
	
	//dilate : réaugmente la taille des formes noirs restantes
	dilate(difference, difference, cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(b, b)));

	//dilate : augmente la taille des formes pour réunir les formes proches entre elles																							  //permet d'éviter que 1 zone soit découper en 2 sous zones
	dilate(difference, difference, cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(c, c)));
	
	//erode : diminue la taille des formes pour revenir approximativement à la taille d'origine
	erode(difference, difference, cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(d, d)));
...
/*===============================================================================================
=======DÉTÉCTIONS DES CONTOURS DES FORMES PUIS DES COORDONNÉES DE LEURS CENTRE DE GRAVITÉ========
=================================================================================================*/
...
Mat contours_(Mat img, Mat diff)
{
	...
	cv::findContours(diff, contours, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, cv::Point(0, 0)); //Determination des contours
	std::vector<cv::Moments> mu(contours.size());
	
	for (int i = 1; i < contours.size(); i++) //calcul du moment pour chaque forme
	{
		if (arcLength(contours[i], true) > 30)
		{
	...
	for (int i = 1; i < 5; i++)//  Obtention du centre de gravité pour chaque forme
	{
		mc[i] = cv::Point2f(mu[i].m10 / mu[i].m00, mu[i].m01 / mu[i].m00);

	}

	cv::Point2f inputQuad[4];  //Coordonnées des centre de gravité dans l'image
	...
	//Calcul des normes entre l'origine de l'image et les centres de gravité
	int cpt_norm[4];
	cv::Mat little_frame;
	double norm[4];
	...
	//Repartition des centres de gravité pour les adapter à la fonction
